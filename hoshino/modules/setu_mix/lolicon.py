...
def get_setu_native(r18 = 0, uid = 0):
    ...
    if uid == 0:
        fn = random.choice(os.listdir(r'C:\XCW\res\setu_mix\lolicon'))
    ...
    res = R.img(path)
    try:
        with open("C:/XCW/res/{}.jpg".format(path), 'rb') as f:
            image['data'] = f.read()
        with open("C:/XCW/res/{}.json".format(path), encoding='utf8') as f:
            d = json.load(f)
            if 'title' in d:
                image['title'] = d['title']
            if 'url' in d:
